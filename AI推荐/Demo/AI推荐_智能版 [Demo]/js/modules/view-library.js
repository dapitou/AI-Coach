    ...
    renderActionDetail: () => {
        ...
        if (tab === 'intro') {
            ...
            html = `
                ...
                <div class="ad-section" id="ad-sec-desc"><div class="ad-sec-title">${window.I18n.t('detail_section_desc')}</div><div class="desc-box">${window.I18n.t('detail_desc_template').replace('{name}', action.name).replace('{part}', action.part).replace('{muscle}', action.muscle).replace('{construct}', action.construct || 'Compound').replace('{func}', action.func.join(' & ')).replace('{diff}', action.difficulty)}</div></div>
            `;
        } else if (tab === 'teach') {
            const teachContent = window.I18n.t('detail_teach_content').replace('{muscle}', action.muscle);
            html = `
                <div class="ad-video"><div style="font-size:12px;">${window.I18n.t('detail_video_placeholder_teach')}</div></div>
                <div class="ad-section" id="ad-sec-points"><div class="ad-sec-title">${window.I18n.t('detail_section_points')}</div><ul style="font-size:13px; color:#ccc; padding-left:20px; margin-bottom:20px; line-height:1.8;">${teachContent}</ul></div>
