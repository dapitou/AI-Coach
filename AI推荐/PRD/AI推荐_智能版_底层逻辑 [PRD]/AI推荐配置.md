...
| 计划周期 (周) | 阶段结构 (按序) | 时长分配权重 (冒号分隔) |
| :--- | :--- | :--- |
| **1** | `[进阶期]` | `1` |
| **2** | `[适应期, 进阶期]` | `1:1` |
| **3** | `[适应期, 进阶期, 突破期]` | `1:1:1` |
| **4 (标准)** | `[适应期, 进阶期, 突破期, 减载期]` | `1:1:1:1` |
| **> 4** | `[适应期, 进阶期, 突破期, 减载期]` | `25:40:25:10` |
...
| 训练范式 | 课程类型映射 | 循环模式 | 负荷策略 | 休息配置 | 默认组数 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **抗阻范式** | 力量, 康复, 高尔夫, 普拉提 | 常规组 | 推荐 | 是 | - |
| **间歇范式** | HIIT, 有氧, 搏击 | 循环组 | 计时 | 是 | - |
| **流式范式** | 瑜伽, 拉伸 | 常规组 | 计时 | 否 | 1 |
...
| 用户等级 | L1概率 | L2概率 | L3概率 | L4概率 | L5概率 | 
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **L1** | 0.80 | 0.20 | 0 | 0 | 0 | 
| **L2** | 0.30 | 0.60 | 0.10 | 0 | 0 | 
| **L3** | 0.10 | 0.20 | 0.60 | 0.10 | 0 | 
| **L4** | 0 | 0.10 | 0.30 | 0.50 | 0.10 | 
| **L5** | 0 | 0 | 0.20 | 0.30 | 0.50 | 
...
### 3.4 RM与百分比换算表 (硬编码)
> **定义**：运动科学通用的 RM (Repetition Maximum) 与强度百分比对应关系表。
> **用途**：用于将训练强度（%）转换为具体的重复次数（Reps），或反之。
> **性质**：基于 NSCA 标准，不作为动态配置下发，仅作逻辑参考。
