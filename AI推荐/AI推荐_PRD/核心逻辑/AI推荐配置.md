...
*   **一、 全局基准配置**
    ...
    *   [1.2 动作模式配比](动作模式配比)
    *   [1.3 结构与组装配置](结构与组装配置)
    *   [1.4 等级强度系数配置](等级强度系数配置)
    *   [1.5 动态循环模板配置](动态循环模板配置)
*   **二、 策略定义配置**
    *   [2.1 负荷与组间策略配置](负荷与组间策略配置)
    *   [2.2 难度兼容策略配置](难度兼容策略配置)
    *   [2.3 阶段周期分配配置](阶段周期分配配置)
*   **三、 推荐与筛选配置**
    *   [3.1 推荐影响因子配置](推荐影响因子配置)
    *   [3.2 风格策略配置](风格策略配置)
*   **四、 状态适配与反馈**
    *   [4.1 状态适配配置](状态适配配置)
    *   [4.2 反馈与自适应配置](反馈与自适应配置)
...
> 数据结构：`映射<模式名称, 模式配方>`
> 用途：定义单节课的动作模式骨架。
> **选择器说明**：优先匹配 `动作模式`，若无则匹配 `动作功能` 或 `动作功率`。
...
```json
{
  "动作模式": "水平推",
  "核心骨架": [ // 占容量大头
    {"维度": "动作模式", "值": "水平推", "容量占比": 0.8}
  ],
  "辅助填充": [ // 占容量小头
    {"维度": "主动肌", "值": "肱三头肌", "容量占比": 0.1},
    {"维度": "主动肌", "值": "三角肌前束", "容量占比": 0.1}
  ]
}
```
...
### 默认配置表
| ID | 名称 | 目标 | 难度 | 周频 | 基础槽位 (摘要) | 填充策略 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **TPL_001** | 一分化 (全身) | 增肌 | L1 | 1 | [全身综合] | 循环 |
| **TPL_002** | 二分化 (上下肢) | 增肌 | L2 | 2 | [上肢日], [下肢日] | 循环 |
| **TPL_003** | 三分化 (推拉腿) | 增肌 | L3 | 3 | [推力日], [拉力日], [腿部日] | 循环 |
| **TPL_004** | 四分化 (非线性) | 增肌 | L4 | 4 | [上肢力量], [下肢力量], [上肢肥大], [下肢肥大] | 循环 |
| **TPL_005** | 五分化 (单部位) | 增肌 | L3 | 5 | [胸], [背], [肩], [腿], [臂] | 循环 |
| **TPL_006** | 减脂循环 | 减脂 | L1 | 3 | [全身HIIT], [有氧耐力], [核心强化] | 循环 |
...
### 4.1 状态适配配置 [合并原4.1+4.2]
> 用途：统一管理基于用户实时状态（疲劳、生理期等）的参数修正与约束。

#### A. 疲劳干预 (Fatigue Adaptation)
> 来源：《疲劳管理逻辑》.部位状态值
...
| :--- | :--- | :--- | :--- |
| **月经期** | 0.60 | 0.50 | 强制剔除[高冲击, 腹部挤压, 倒立]动作；推荐[瑜伽, 拉伸] |
