...
class TextConfig:
    WINDOW_NAME: str = "AEKE Fitness Mirror V101 (V80 Core + Spatial Fix)"
    ACT_PRESS: str = "推举"
    ACT_SQUAT: str = "深蹲"
    ACT_RAISE: str = "前平举"
...
class ColorConfig:
    ...
    NEON_PURPLE: tuple = (255, 0, 255) # 脊柱核心色
...
class AlgoConfig:
    ...
    SPINE_OFFSET_SMOOTH: float = 0.3
    
    # --- V80 核心复刻参数 ---
    # 节点比例 (T7, L3)
    POS_THORAX: float = 0.33 
    POS_LUMBAR: float = 0.64
    
    # 自然曲度 (S型基础)
    # 正值 = 向背侧凸 (Kyphosis)
    # 负值 = 向腹侧凸 (Lordosis)
    NATURAL_KYPHOSIS: float = 0.05
    NATURAL_LORDOSIS: float = -0.08 
    
    # 动态增益 (V80高灵敏度)
    FLEXION_GAIN: float = 6.0
    
    # 判定阈值
    ROUNDING_TH: float = 0.05        # 弓腰判定
    ROUNDING_COMPRESS_TH: float = 0.03 # 压缩响应起始点
    
    # 侧身判定 (膝-髋 像素距离)
    SIDE_VIEW_TH: float = 25.0
    
    # 限制 (ROM Constraint)
    SPINE_LIMIT_RATIO: float = 0.20 # 最大偏移不超过躯干20%
    
    # 校准
    CALIBRATION_RATE: float = 0.01
    
    # 兼容参数 (防止报错)
    WAIST_LOC_RATIO: float = 0.64
...
class ScalarSmoother:
    ...
    def update(self, new_val):
        ...
        else: self.val = self.alpha * new_val + (1 - self.alpha) * self.val
        return self.val
...
class SoundManager:
    ...
    def play(self, name):
        if HAS_AUDIO and name in self.sounds:
            try: self.sounds[name].set_volume(AppConfig.VOL); self.sounds[name].play()
            except: pass
...
class UIRenderer:
    ...
    def draw_skeleton(self, img, pts, is_avatar=False):
        ...
        if pts.get('waist') and pts.get('thorax') and pts.get('neck') and pts.get('hip'):
            st = pts.get('spine_state', 'neutral')
            col = ColorConfig.NEON_PURPLE
            if st == 'rounding': col = ColorConfig.NEON_RED
            elif st == 'arching': col = ColorConfig.NEON_ORANGE
    ...
    def draw_hints(self, img, hints):
        for h in hints:
            ...
            if h['type'] == 'press_guide':
                ...
                else: cv2.arrowedLine(img, (s[0], s[1]), (s[0], s[1]-50), color, 4, 8, 0, 0.4)
            elif h['type'] == 'depth': 
                ...
                if ok: self._draw_check(img, (start_x, pt[1]), ColorConfig.NEON_GREEN)
                else: cv2.arrowedLine(img, (start_x, pt[1]), (start_x, pt[1]+50), ColorConfig.NEON_RED, 4, 8, 0, 0.4)
            elif h['type'] == 'spine_alert':
                w = h['waist']
                if not ok:
                    cv2.circle(img, w, 20, ColorConfig.NEON_RED, 2)
                    cv2.putText(img, "!", (w[0]-8, w[1]+10), cv2.FONT_HERSHEY_SIMPLEX, 1, ColorConfig.NEON_RED, 3)
...
class LogicEngine:
    def __init__(self, sound_mgr):
        ...
        self.smoother = PointSmoother(alpha=AlgoConfig.SMOOTH_ALPHA)
        self.offset_filter = ScalarSmoother(alpha=AlgoConfig.SPINE_OFFSET_SMOOTH)
        self.landmarks_vis = []
    ...
    def update(self, pts, world_pts, vis_scores, user_height_cm):
        ...
        # [核心] V101 双轴联动脊柱
        self._update_bio_spine_v101(pts)
    ...
    def _update_bio_spine_v101(self, pts):
        ...
        if pts.get('lk') and pts.get('rk'):
            kx = (pts['lk'][0]+pts['rk'][0])/2
            k_off = kx - hx
            if abs(k_off) > AlgoConfig.SIDE_VIEW_TH:
                # Knee Left (<0) -> Face Left -> Back Right
                # Normal points Left (-1).
                # Back Right is -Normal. So back_dir = -1.
                if k_off < 0: back_dir = -1
                else: back_dir = 1
        ...
        if back_dir != 0:
            ...
            # 胸: 后凸 (Back) -> +back_dir
            # 腰: 前凸 (Front) -> -back_dir
            t_val = AlgoConfig.NATURAL_KYPHOSIS * curr_len * back_dir
            l_val = -AlgoConfig.NATURAL_LORDOSIS * curr_len * back_dir # Lordosis is neg? no, let's simply use sign.
            # Config: Lordosis = -0.08 (Neg).
            # So l_val = -(-0.08) * back = +0.08 * back (Backwards?) -> WRONG.
            # Lordosis should be Front (-Back).
            # So l_val = Config(-0.08) * len * back_dir -> Points Front?
            # Let's say back_dir=1 (Back is Left). l_val = -0.08. -0.08 * Normal(Left) = Right(Front). Correct.
            l_val = AlgoConfig.NATURAL_LORDOSIS * curr_len * back_dir
            
            # B. 动态弓背 (Flexion)
            if compression > AlgoConfig.ROUNDING_COMPRESS_TH:
                dyn = compression * curr_len * AlgoConfig.FLEXION_GAIN
                # 叠加方向: Back (+back_dir)
                l_val += dyn * back_dir # 抵消前凸
                t_val += dyn * 0.3 * back_dir
        
        # 平滑
        l_val = self.offset_filter.update(l_val)
        
        # 限制
        limit = curr_len * AlgoConfig.SPINE_LIMIT_RATIO
        t_val = np.clip(t_val, -limit, limit)
        l_val = np.clip(l_val, -limit, limit)
        
        # 5. 坐标生成 (双轴联动)
        # 始终基于中轴线 (sx,sy)->(hx,hy) 进行偏移
        # 这保证了正面侧屈时，脊柱随中轴线旋转，不会乱飞
        
        tx = sx + vx * AlgoConfig.POS_THORAX + nx * t_val
        ty = sy + vy * AlgoConfig.POS_THORAX + ny * t_val
        lx = sx + vx * AlgoConfig.POS_LUMBAR + nx * l_val
        ly = sy + vy * AlgoConfig.POS_LUMBAR + ny * l_val
        ...
        # 6. 状态
        is_rounding = False
        if back_dir != 0 and (l_val * back_dir) > (curr_len * AlgoConfig.ROUNDING_TH):
            is_rounding = True
