...
class AlgoConfig:
    ...
    # [弓步蹲 - 核心升级]
    # 门控：双脚直线距离系数
    LUNGE_STANCE_RATIO: float = 0.8 
    # 门控：双脚Y轴错层阈值 (px)，用于判定前后分腿
    LUNGE_Y_DIFF_TH: int = 50
    
    # 计次 (髋膝绝对位置)
    LUNGE_DOWN_OFFSET: int = 100 # 髋部高于膝盖100px以内算蹲下
    LUNGE_UP_OFFSET: int = 200   # 髋部高于膝盖200px以上算站起
    
    # 纠错
    LUNGE_KNEE_TOLERANCE: int = 15 
    # 内扣检测深度补偿 (髋部接近膝盖 150px 时才开始检测)
    LUNGE_VALGUS_CHECK_OFFSET: int = 150
...
@dataclass(frozen=True)
class TextConfig:
    WINDOW_NAME: str = "AEKE Fitness Mirror V2.7.0 (Lunge Pro Logic)"
    ...
    def draw_hints(self, img, hints):
        for h in hints:
            ...
            if h['type'] == 'press_guide':
                ...
                k, a, f = h['knee'], h['ankle'], h['foot_mid']
                direction = h['direction'] 
                
                target_y = k[1] - 100
                self._draw_dash(img, f, (f[0], target_y), color)
...
class LogicEngine:
    ...
    def _determine_facing_direction(self, pts):
        score = 0.0
    ...
    def _gatekeeper(self, pts):
        ...
        if self.mode == TextConfig.ACT_PRESS:
            ...
            if pts.get('lt') and pts.get('rt') and pts.get('ls') and pts.get('rs'):
                ...
                # [New] Y轴错层检测：确保是一前一后站立
                y_diff = abs(pts['lt'][1] - pts['rt'][1])
                
                if foot_dist > shoulder_w * AlgoConfig.LUNGE_STANCE_RATIO and y_diff > AlgoConfig.LUNGE_Y_DIFF_TH:
                    is_similar = True
    ...
    def _lunge(self, p, vis, scale):
        ...
        # 获取前腿关键点
        fk = p['lk'] if is_left_front else p['rk'] # Front Knee
        fh = p['lh'] if is_left_front else p['rh'] # Front Hip
        
        # 2. 状态机 (Absolute Threshold)
        # DOWN: 前腿髋部Y > 前腿膝盖Y - 100 (接近水平)
        if self.stage != "down" and fh[1] > fk[1] - AlgoConfig.LUNGE_DOWN_OFFSET:
            self.stage = "down"
            self._reset_flags(["lunge_knee"])
            self.current_rep_has_error = False
            # 锁定前腿ID (虽然这里每帧算，但动作过程中透视关系不变)
            self.lunge_front_idx = 25 if is_left_front else 26
            
        # UP: 前腿髋部Y < 前腿膝盖Y - 200 (回到站立)
        elif self.stage == "down" and fh[1] < fk[1] - AlgoConfig.LUNGE_UP_OFFSET:
            self.stage = "up"
            self.counter += 1
            self._end_cycle(["lunge_knee"])
            self.lunge_front_idx = None
        ...
        # 检查是否在检测区域 (比如已经蹲了一半)
        in_check_zone = fh[1] > fk[1] - AlgoConfig.LUNGE_VALGUS_CHECK_OFFSET
        
        if in_check_zone:
            fa = p['la'] if is_left_front else p['ra']
            ft, fheel = p['lt'], p['lhe']
            if not is_left_front: ft, fheel = p['rt'], p['rhe']
            
            foot_mid_x = (ft[0] + fheel[0]) / 2
            foot_mid_pt = (int(foot_mid_x), fa[1])
            
            # 左腿前(User Left, Screen Right) -> 内扣向左(-X)
            if is_left_front:
        ...
        # 状态更新 (如果在检测区内发生内扣，标记错误)
        # 注意：这里是反逻辑，flag=False代表有错
        if self.stage == "down" and in_check_zone and is_valgus:
            self.cycle_flags['lunge_knee'] = False

    def _front_raise(self, p, wp, vis, scale):
        if not (p.get('lw') and p.get('rw') and p.get('le') and p.get('re') and p.get('ls') and p.get('rs') and p.get('lh') and p.get('rh')): return

        wrist_y = (p['lw'][1] + p['rw'][1]) / 2
        shou_y = (p['ls'][1] + p['rs'][1]) / 2
        hip_y = (p['lh'][1] + p['rh'][1]) / 2 
        
        start_th = hip_y - 100
        end_th = hip_y - 50
        
        if self.stage != "up" and wrist_y < start_th:
            self.stage = "up"
            self._reset_flags(["range", "shrug"])
            self.current_rep_has_error = False
        elif self.stage == "up" and wrist_y > end_th:
            self.stage = "down"
            self.counter += 1
            self._end_cycle(["range", "shrug"])

        if self.stage == "up":
            if p['le'][1] <= p['ls'][1] or p['re'][1] <= p['rs'][1]:
        ...
        if 'range' in self.active_errs:
            ...
            vis.append({'type':'raise_guide', 'shoulder':p['ls'], 'elbow':p['le'], 'side':'left', 'ok':self.cycle_flags['range']})
            vis.append({'type':'raise_guide', 'shoulder':p['rs'], 'elbow':p['re'], 'side':'right', 'ok':self.cycle_flags['range']})
    ...
    def _squat(self, p, vis, scale):
        ...
        if self.stage == "down" and is_descending and in_valgus_check_zone:
