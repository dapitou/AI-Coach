...
@dataclass(frozen=True)
class TextConfig:
    WINDOW_NAME: str = "AEKE Fitness Mirror V2.4.2 (Visual Polish)"
...
class UIRenderer:
    ...
    # [新] 通用箭头特效引擎：自动计算间距、方向和弹性动画
    def _draw_dynamic_arrow(self, img, start, direction_vec, color, gap=25, base_len=50):
        """
        start: 起点坐标 (x, y)
        direction_vec: 方向向量 (dx, dy) 或 目标点坐标 (tx, ty)
        gap: 箭头起点与关键点的距离 (px)
        base_len: 箭头基础长度 (px)
        """
        if isinstance(direction_vec, tuple) and len(direction_vec) == 2:
            # 如果输入是目标点，先计算向量
            dx, dy = direction_vec[0] - start[0], direction_vec[1] - start[1]
        else:
        ...
        for h in hints:
            ...
            if h['type'] == 'press_guide':
                ...
                if not ok:
                    # 引导：从手腕水平指向目标线
                    # 向量: (目标X - 手腕X, 0)
                    self._draw_dynamic_arrow(img, s, (target[0], s[1]), ColorConfig.NEON_RED, gap=20)
                else:
            ...
            # [PRD] 膝内扣
                ...
                if ok: 
                    ...
                    # 引导：向身体外侧指
                    # 左膝(屏幕左侧) -> 向左(-1, 0); 右膝 -> 向右(1, 0)
                    vec = (-1, 0) if side == 'left' else (1, 0)
                    self._draw_dynamic_arrow(img, pt, vec, ColorConfig.NEON_RED, gap=20, base_len=60)
            ...
            # [PRD] 弓背
                ...
                # 终点：腰部
                # 这里逻辑稍微反转：箭头像是"外力"在推腰
                self._draw_dynamic_arrow(img, start_arrow, target_pt, ColorConfig.NEON_ORANGE, gap=0, base_len=60)
            ...
            # [PRD] 前平举
                ...
                if not ok:
                    # 引导：从肘部垂直向上
                    self._draw_dynamic_arrow(img, e, (0, -1), ColorConfig.NEON_RED, gap=20)
                else:
