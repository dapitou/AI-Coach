...
class AlgoConfig:
    ...
    # [深蹲内扣判定] 膝宽 < 髋宽 * 1.1 (深蹲站距通常略宽于肩，膝盖若小于髋宽则内扣严重)
    VALGUS_RATIO: float = 1.1 
    ...
    # [弓步内扣判定] 前膝偏离中线 < (1/2髋宽) * 0.8
    LUNGE_VALGUS_RATIO: float = 0.8 
    LUNGE_DOWN_TH: int = 40   
    LUNGE_UP_TH: int = 100    
...
@dataclass(frozen=True)
class TextConfig:
    WINDOW_NAME: str = "AEKE Fitness Mirror V2.15.2 (Crash & Logic Fix)"
    ACT_PRESS: str = "推举"
    ACT_SQUAT: str = "深蹲"
    ACT_RAISE: str = "前平举"
    ACT_LUNGE: str = "弓步蹲"
...
class UIRenderer:
        ...
        for h in visuals:
            ...
            if tag == 'press_guide':
                ...
                if not ok:
                    # 指向垂直线
                    vec = (target[0]-s[0], 0)
                    self._draw_dynamic_arrow(img, s, vec, ColorConfig.NEON_RED, gap=20, mode='point')
                else:
                    self._draw_check(img, (s[0], s[1]-30), ColorConfig.NEON_GREEN, 1.2)

            elif tag == 'squat_valgus':
                pt, side = h['pt'], h['side']
                feet_exist = h.get('foot_pt') is not None
                ...
                self._draw_dash(img, h['p1'], h['p2'], col)
                cv2.circle(img, h['p1'], 8, col, -1)
                cv2.circle(img, h['p2'], 8, col, 2)
            
            # 兼容旧 bounce_arrow (已废弃，保留防止报错)
            elif tag == 'bounce_arrow':
                pt, side = h['start'], h['side']
                if ok: self._draw_check(img, (pt[0], pt[1]-30), ColorConfig.NEON_GREEN)
                else:
                    center_x = AppConfig.W // 4 
                    vec = (-1, 0) if pt[0] < center_x else (1, 0)
                    self._draw_dynamic_arrow(img, pt, vec, ColorConfig.NEON_RED, gap=20, base_len=60, mode='vector')
            ...
            elif tag == 'lunge_depth':
                m, t = h['mid'], h['target']
                self._draw_dash(img, m, t, col)
                cv2.circle(img, m, 8, col, -1)
                cv2.circle(img, t, 12, col, 2)
    ...
    def _txt(self, draw, x, y, label, val, small=False, col=None, val_col=None, font=None, is_acc=False):
        f = font if font else (self.font_sm if small else self.font_sm)
        vf = self.font_md if small else self.font_lg
        c = col if col else ColorConfig.TEXT_DIM
        draw.text((x+1, y+1), label, font=f, fill=(0,0,0))
        draw.text((x, y), label, font=f, fill=c)
        if val: 
            vc = val_col if val_col else ColorConfig.NEON_YELLOW
            draw.text((x+80+1, y-10+1), val, font=vf, fill=(0,0,0))
...
class LogicEngine:
    ...
    def _gatekeeper(self, pts):
        ...
        if self.mode == TextConfig.ACT_PRESS:
            ...
            if pts.get('lt') and pts.get('rt') and pts.get('ls') and pts.get('rs'):
                foot_dist = GeomUtils.dist(pts['lt'], pts['rt'])
                shoulder_w = GeomUtils.dist(pts['ls'], pts['rs'])
                if shoulder_w > 0 and foot_dist > shoulder_w * AlgoConfig.LUNGE_STANCE_X_RATIO:
                    is_similar = True
    ...
    def _front_raise(self, p, wp, vis, scale):
        ...
        sy = (p['ls'][1]+p['rs'][1])/2
        ty = (p['le'][1]+p['re'][1])/2
        if self.stage != "up" and wy < ty:
            self.stage = "up"; self._reset_flags(["range","shrug"]); self.current_rep_has_error = False
        elif self.stage == "up" and wy > sy + 100:
            self.stage = "down"; self.counter += 1; self._end_cycle(["range","shrug"])
    ...
    def _press(self, p, vis, scale):
        ...
        if self.stage != "up" and wrist_y < nose_y - 50:
            self.stage = "up"; self._reset_flags(["arm","shrug"]); self.current_rep_has_error = False
        elif self.stage == "up" and wrist_y > nose_y:
            self.stage = "down"; self.counter += 1; self._end_cycle(["arm","shrug"])
    ...
    def _squat(self, p, vis, scale):
        ...
        # 1. 强制初始化变量 (防止 UnboundLocalError)
        in_valgus_check_zone = False
        ...
        # [修改] 深蹲内扣判定: 膝距 < 踝距 * 0.95 (容错处理)
        knee_dist = abs(p['lk'][0] - p['rk'][0])
        ankle_dist = abs(p['la'][0] - p['ra'][0])
        is_valgus = knee_dist < (ankle_dist * AlgoConfig.VALGUS_RATIO)
        
        # 1/3 深度检测
        in_valgus_check_zone = hy > ky - 200
        
        if desc and in_valgus_check_zone:
        ...
        if 'valgus' in self.active_errs:
            ...
            # 准备足部坐标用于绘制辅助线
            l_foot_center = ((p['la'][0] + p['lt'][0])//2, (p['la'][1] + p['lt'][1])//2) if p.get('lt') else None
            r_foot_center = ((p['ra'][0] + p['rt'][0])//2, (p['ra'][1] + p['rt'][1])//2) if p.get('rt') else None
            ...
            vis.append({'type':'depth', 'p1':((p['lh'][0]+p['rh'][0])//2, int(hy)), 'p2':((p['lk'][0]+p['rk'][0])//2, int(ky)), 'ok':deep})
    ...
    def _lunge(self, p, vis, scale):
        # 1. 变量强制初始化
        is_descending = False
        is_valgus = False
        
        if not (p.get('lh') and p.get('rh') and p.get('lk') and p.get('rk') and 
                p.get('la') and p.get('ra') and p.get('lt') and p.get('rt') and p.get('lhe') and p.get('rhe')): 
            return
        
        is_left_front = p['la'][1] > p['ra'][1]
        current_front_idx = 25 if is_left_front else 26
        idx = self.lunge_front_idx if self.lunge_front_idx is not None else current_front_idx
        
        fk = p['lk'] if idx == 25 else p['rk']
        fh = p['lh'] if idx == 25 else p['rh']
        fa = p['la'] if idx == 25 else p['ra']
        ft = p['lt'] if idx == 25 else p['rt'] # 足尖
        ...
        if self.stand_hip_y == 0 or hy < self.stand_hip_y:
            self.stand_hip_y = hy

        if self.stage != "down" and hy > ky - AlgoConfig.LUNGE_DOWN_TH:
            self.stage = "down"
            self._reset_flags(["lunge_valgus", "lunge_depth"])
            self.current_rep_has_error = False
            self.lunge_front_idx = current_front_idx
        elif self.stage == "down" and hy < ky - AlgoConfig.LUNGE_UP_TH:
            self.stage = "up"
            self.counter += 1
        ...
        if self.stage == "down" and is_deep:
            self.cycle_flags['lunge_depth'] = True
            
        # [修改] 弓步内扣判定：基于髋中点垂线
        hip_mid_x = (p['lh'][0] + p['rh'][0]) / 2
        fk_x = fk[0]
        fa_x = (p['la'][0] if idx==25 else p['ra'][0])
        
        knee_off = abs(fk_x - hip_mid_x)
        ankle_off = abs(fa_x - hip_mid_x)
        
        # 判定：如果膝盖距中线 < 脚踝距中线 * 0.95，则认为内扣
        is_valgus = knee_off < ankle_off * AlgoConfig.LUNGE_VALGUS_RATIO
        
        check_depth_y = ky - AlgoConfig.LUNGE_VALGUS_CHECK_OFFSET
        if self.stage == "down" and is_descending and hy > check_depth_y:
            if is_valgus: self.cycle_flags['lunge_valgus'] = False
            
        if 'lunge_valgus' in self.active_errs:
            self._set_msg(TextConfig.ERR_LUNGE_KNEE, ColorConfig.NEON_RED, perm=True)
            d = 'right' if idx == 25 else 'left'
            
            # 计算足中点用于绘制垂线
            foot_center = ((fa[0] + ft[0])//2, (fa[1] + ft[1])//2)

            if self.stage == "down":
                vis.append({'type':'lunge_knee_guide', 'knee':fk, 'foot_pt': foot_center, 'direction':d, 'ok':not is_valgus})

        elif 'lunge_depth' in self.active_errs:
            self._set_msg(TextConfig.ERR_LUNGE_DEPTH, ColorConfig.NEON_RED, perm=True)
            hm = ((p['lh'][0]+p['rh'][0])//2, int(hy))
            tg = (hm[0], int(ky))
            vis.append({'type':'lunge_depth', 'mid':hm, 'target':tg, 'ok':is_deep})
