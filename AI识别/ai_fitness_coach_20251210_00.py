...
ERR_NAMES_MAP = {
    ...
    'depth': '深度',
    'range': '行程',
    'lunge_valgus': '前膝' # 移除 lunge_depth
}
...
class AlgoConfig:
    ...
    # [弓步蹲 - 相对位移阈值]
    LUNGE_DROP_DOWN_RATIO: float = 0.15 
    LUNGE_DROP_UP_RATIO: float = 0.10   
...
@dataclass(frozen=True)
class TextConfig:
    WINDOW_NAME: str = "AEKE Fitness Mirror V2.21.0 (Lunge Standardized)"
    ACT_PRESS: str = "推举"
    ACT_SQUAT: str = "深蹲"
    ACT_RAISE: str = "前平举"
    ACT_LUNGE: str = "弓步蹲"
...
class UIRenderer:
        ...
        for h in visuals:
            ...
            if tag == 'press_guide':
                ...
                if not ok:
                    ...
                    # [优化] 达标时显示绿色对勾
                    self._draw_check(img, (s[0], s[1]-30))
    ...
    def draw_all_text_layers(self, img, mode, count, fps, menu, h_str, typing, msg, msg_color, error_stats, bad_reps, vid, paused):
        ...
        rx_stat = rx_acc - 180
        sorted_errs = sorted([(k, v) for k, v in error_stats.items() if v > 0], key=lambda item: item[1], reverse=True)
        for err_key, err_val in sorted_errs:
            err_name = ERR_NAMES_MAP.get(err_key, "未知")
            if err_val > 0:
...
class LogicEngine:
    ...
    def _gatekeeper(self, pts):
        ...
        # [Rule] If active, always pass
        if self.stage == "down": return True
    ...
    def _press(self, p, vis, scale):
        ...
        if 'arm' in self.active_errs:
            ...
            # [Fix] Need current frame status
            lok = GeomUtils.is_vertical(p['lw'], p['le'], AlgoConfig.PRESS_VERT_TOLERANCE)
            rok = GeomUtils.is_vertical(p['rw'], p['re'], AlgoConfig.PRESS_VERT_TOLERANCE)
            vis.append({'type':'press_guide', 'elbow':p['le'], 'wrist':p['lw'], 'ok':lok})
            vis.append({'type':'press_guide', 'elbow':p['re'], 'wrist':p['rw'], 'ok':rok})
        elif 'shrug' in self.active_errs:
            self._set_msg(TextConfig.ERR_PRESS_SHRUG, ColorConfig.NEON_RED, perm=True)
    ...
    def _squat(self, p, vis, scale):
        ...
        if 'valgus' in self.active_errs:
            ...
            # [Fix] Handle missing foot points safely
            l_foot_center = ((p['la'][0] + p['lt'][0])//2, p['lt'][1]) if p.get('lt') else None
            r_foot_center = ((p['ra'][0] + p['rt'][0])//2, p['rt'][1]) if p.get('rt') else None
    ...
    def _lunge(self, pts, vis, scale):
        ...
        valgus = False
        direction = 'left'
        fheel = pts.get('lhe') if idx == 25 else pts.get('rhe')
        ...
        if self.front_idx:
            ...
            # [Trigger] Correct logic: set error flag and sound
            ...
            # [Fix] Real-time release
            if self.stage == "down" and not valgus and 'lunge_valgus' in self.active_errs:
                self.active_errs.remove('lunge_valgus')
                # Trigger green encouragement immediately
                self._set_msg(TextConfig.MSG_GOOD, ColorConfig.NEON_GREEN, 1.0)
