...
class TextConfig:
    WINDOW_NAME: str = "AEKE Fitness Mirror V66 (Interaction Fixed)"
...
class AlgoConfig:
    ...
    RAISE_HEIGHT_RATIO: float = 0.5 # 躯干高度的0.5倍
...
class UIRenderer:
    ...
    def update_hover(self, x, y, menu_open):
        ...
        if menu_open:
            # 下拉菜单项 (3个动作)
            for i in range(3):
                # 75是起始Y, 55是行高 (50h + 5gap)
                y_start = 75 + i * 55
                y_end = y_start + 50
                self.hover[f"i{i}"] = (20 <= x <= 240 and y_start <= y <= y_end)
        else:
...
class LogicEngine:
    ...
    def _check_shrug_adaptive(self, p, scale, stage_check="up"):
        ...
        if self.stage == stage_check and self.base_shrug_dist > 0:
            ...
            if ratio < AlgoConfig.SHRUG_RATIO_TH:
                self.cycle_flags['shrug'] = False
    ...
    def mouse_cb(e, x, y, f, p):
        ...
        if e == cv2.EVENT_LBUTTONDOWN:
            ...
            if menu_open:
                y_base = 75
                clicked_item = False
                for i, m in enumerate([TextConfig.ACT_PRESS, TextConfig.ACT_SQUAT, TextConfig.ACT_RAISE]):
                    if 20 < x < 240 and y_base < y < y_base + 50:
                        engine.set_mode(m)
                        menu_open = False
                        clicked_item = True
                        break
            ...
            # 3. 检查菜单按钮点击 (或者点击空白处关闭菜单)
            elif 20 < x < 240 and 20 < y < 70:
