...
class TextConfig:
    WINDOW_NAME: str = "AEKE Fitness Mirror V84 (Data Flow Fixed)"
    ACT_PRESS: str = "推举"
    ACT_SQUAT: str = "深蹲"
    ACT_RAISE: str = "前平举"
...
class LogicEngine:
    ...
    def _update_elastic_spine(self, pts):
        ...
        # 强制计算默认点位 (应对无耳数据)
        ratio = AlgoConfig.WAIST_LOC_RATIO
        bx = sx + vx * ratio
        by = sy + vy * ratio
        ...
        if pts.get('le_ear') and pts.get('re_ear'):
            ...
            if compression > AlgoConfig.ROUNDING_COMPRESS_TH and abs(dist_signed) > 5:
                ...
                l_base = -AlgoConfig.NATURAL_LORDOSIS * curr_len
                l_nat = (l_base + dyn) * back_dir
            else:
        ...
        if not has_ear:
            pts['spine_state'] = 'neutral'
            pts['rounding_bad'] = False
            pts['thorax'] = (int(sx+vx*0.33), int(sy+vy*0.33))
            pts['lumbar'] = (int(sx+vx*0.66), int(sy+vy*0.66))
            pts['waist'] = pts['lumbar']
    ...
    def _check_shrug_adaptive(self, p, scale, stage_check="up"):
        ...
        self.neck_curr_smooth = curr_dist # pts already smoothed
...
def main():
    ...
    while True:
        ...
        if res.pose_landmarks:
            ...
            # [修复] 捕获两个返回值
            hints, smooth_pts = engine.update(pts, world_pts, vis_scores, h_val)
