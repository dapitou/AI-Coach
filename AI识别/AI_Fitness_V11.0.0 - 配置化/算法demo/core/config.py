"""
系统配置模块 V24.0.0 (Ratio Optimization)
"""
from dataclasses import dataclass

ERR_NAMES_MAP = {
    'shrug': '耸肩', 'arm': '小臂不垂直', 'valgus': '膝盖内扣', 'rounding': '弓背',
    'depth': '下蹲过浅', 'range': '幅度不足', 'lunge_valgus': '前膝内扣', 
    'lunge_depth': '下蹲幅度', 'lunge_knee': '膝盖过伸'
}
...
class AlgoConfig:
    ...
    # --- 视图门控 ---
    VIEW_SIDE_TH: float = 0.38
    VIEW_FRONT_TH: float = 0.48
    
    # --- 纯粹漂移补偿参数 ---
    ENABLE_HIP_DRIFT_COMP: bool = True
    HIP_DRIFT_GAIN: float = 0.15
    
    # --- 阻尼手感参数 ---
    SPINE_DAMPING_LIMIT: float = 0.18
    
    # 视觉幅度
    ROUNDING_AMP: float = 6.5  
    ARCHING_AMP: float = 6.0
    ...
    ENABLE_TRAJECTORY: bool = True # [New] 全局轨迹开关
    ...
    # [New] 压缩率阈值 (替代原像素阈值)
    # 物理含义: 当前髋膝垂直距离 / 站立基准距离
    SQUAT_DOWN_RATIO: float = 0.85   # 开始下蹲: 缩短至85%
    SQUAT_UP_RATIO: float = 0.92     # 判定起立: 恢复至92%
    SQUAT_DEPTH_RATIO: float = 0.15  # 深度达标: 距离小于15% (髋部接近膝盖)
...
class TextConfig:
    WINDOW_NAME: str = "AEKE Fitness Mirror V24.0.0 (Visual Direction Fix)"
    ACT_PRESS: str = "推举"
    ACT_SQUAT: str = "深蹲"
    ACT_RAISE: str = "前平举"
    ACT_LUNGE: str = "弓步蹲"
    ...
    TextConfig.ACT_SQUAT: [
        ...
        },
        {
            ...
            'params': [
                ('ROUNDING_AMP', '弓背幅度'),
                ('ARCHING_AMP', '挺腰幅度'),
                ('SPINE_DAMPING_LIMIT', '阻尼松紧度(0-1)'),
                ('ENABLE_HIP_DRIFT_COMP', '俯身补偿开关'),
                ('HIP_DRIFT_GAIN', '髋漂移补偿')
            ]
        },
        {
            'name': '膝盖内扣检测',
        ...
        {
            'name': '计次与深度(压缩率)', # [Mod] 优化后的名称
            'switch': None,
            'params': [
                # 替换原像素参数 -> 新的比率参数
                ('SQUAT_DOWN_RATIO', '下蹲触发比例'),
                ('SQUAT_UP_RATIO', '起立触发比例'),
                ('SQUAT_DEPTH_RATIO', '深度达标比例')
            ]
        }
    ],
...
PARAM_TIPS = {
    'ENABLE_HIP_DRIFT_COMP': "俯身补偿总开关。\n开启：计算包含AI误差修正后的物理压缩比。\n关闭：计算原始测量的几何压缩比。",
    'HIP_DRIFT_GAIN': "髋漂移补偿系数 (0.15)。\n仅在正向俯身(>0°)时生效。",
    'SPINE_DAMPING_LIMIT': "阻尼饱和极限 (0.18)。",
    'ROUNDING_AMP': "弓背放大系数 (5.0)。",
    'ARCHING_AMP': "挺腰放大系数 (5.0)。",
    'ENABLE_TRAJECTORY': "开关：是否显示关节运动轨迹流体特效。",
    ...
    # [New] 新参数提示
    'SQUAT_DOWN_RATIO': "下蹲触发灵敏度。\n当大腿垂直投影长度 < 基准值 * 0.85 时，开始计次。",
    'SQUAT_UP_RATIO': "起立判定灵敏度。\n当大腿垂直投影长度 > 基准值 * 0.92 时，认为已起立。",
    'SQUAT_DEPTH_RATIO': "深度达标阈值。\n当髋膝垂直间距 < 基准值 * 0.15 时，判定深度合格。",
}