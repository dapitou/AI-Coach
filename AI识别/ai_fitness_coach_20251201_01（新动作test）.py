...
class AlgoConfig:
    ...
    LUNGE_DEPTH_RATIO: float = 0.15 
    LUNGE_CHECK_DEPTH: float = 0.20 
...
@dataclass(frozen=True)
class TextConfig:
    WINDOW_NAME: str = "AEKE Fitness Mirror V2.6.5 (Front Raise Visuals)"
    ...
    def draw_hints(self, img, hints):
        for h in hints:
            ...
            # [修复] 前平举交互：水平虚线 + 垂直箭头
                ...
                cv2.circle(img, target_pt, 10, color, 2) # 目标圈
                
                if not ok:
                    # 引导：从肘部垂直向上指
                    self._draw_dynamic_arrow(img, e, (0, -1), ColorConfig.NEON_RED, gap=20, mode='vector')
                    cv2.circle(img, e, 8, ColorConfig.NEON_RED, -1) # 肘部实心点
                else:
...
class LogicEngine:
    ...
    def _gatekeeper(self, pts):
        ...
        if self.mode == TextConfig.ACT_PRESS:
            ...
            if pts.get('lt') and pts.get('rt') and pts.get('ls') and pts.get('rs'):
                ...
                if foot_dist > shoulder_w * AlgoConfig.LUNGE_STANCE_RATIO:
                    is_similar = True
    ...
    def _front_raise(self, p, wp, vis, scale):
        ...
        if self.stage == "up":
            # 如果肘部Y > 肩部Y (低于肩部)，则记录为 True (保持错误状态)
            # 只要有一瞬间 肘部Y <= 肩部Y，则清除错误
            if p['le'][1] <= p['ls'][1] or p['re'][1] <= p['rs'][1]: # 任意一侧达标即可
    ...
    def _lunge(self, p, vis, scale):
        ...
        ref_len = self.max_torso_len if self.max_torso_len > 0 else 200
        drop_dist = hy - self.stand_hip_y
        
        is_descending = hy > (self.prev_hip_y + 2)
        self.prev_hip_y = hy

        if self.stage != "down" and drop_dist > ref_len * AlgoConfig.LUNGE_DEPTH_RATIO:
            self.stage = "down"
            self._reset_flags(["lunge_knee"])
            self.current_rep_has_error = False
            if p['la'][1] > p['ra'][1]: self.lunge_front_idx = 25 
            else: self.lunge_front_idx = 26 
            
        elif self.stage == "down" and drop_dist < ref_len * AlgoConfig.LUNGE_RETURN_RATIO:
            self.stage = "up"
            self.counter += 1
        ...
        if self.lunge_front_idx is not None:
            if self.lunge_front_idx == 25:
                fk, fa, ft, fh = p['lk'], p['la'], p['lt'], p['lhe']
            else:
                fk, fa, ft, fh = p['rk'], p['ra'], p['rt'], p['rhe']
            
            foot_mid_x = (ft[0] + fh[0]) / 2
        ...
        in_check_zone = drop_dist > ref_len * AlgoConfig.LUNGE_CHECK_DEPTH
        if self.stage == "down" and is_descending and in_check_zone and is_valgus:
